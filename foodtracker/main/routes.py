...
def create_log():
    ...
    db.session.add(log)
    db.session.commit()
    return redirect(url_for('main.view', log_id=log.log_id))
...
def add_post():
    foodname=request.form.get('food-name')
    ...
    if food_id:
        ...
        new_food=Food(name=foodname, proteins=proteins, carbs=carbs, fats=fats)
...
def edit_food(food_id):
    ...
    return render_template('add.html', food=food, foods=foods, log=log)
